...
| `event_hooks` | boolean |  | `true` | EventHooks controls installation of the bead event-forwarding hooks (.beads/hooks/on_create,on_update,on_close) that shell out to `gc event emit` on every bead write. Defaults to true. Set to false once the controller's cache-events already observe bead changes: the lifecycle then removes the event hooks (leaving git hooks untouched) and stops reinstalling them, clearing the per-write churn. |
...
| `server` | DoltConfig |  |  | Server configures the beads storage server's operational tuning (the dolt sql-server bd manages via proxied-server): archive level, auto-GC, max connections, and read/write timeouts. gascity is dolt-agnostic — this is the [beads.server] table; the endpoint itself is owned by bd. |
