...
#
# Environment: GC_CITY_PATH, GC_BEADS_PORT (legacy mode), GC_BEADS_USER,
#              GC_BEADS_PASSWORD, GC_BEADS_PULL_TIMEOUT_SECS (default 1800)
#
...
# prevent).
pull_timeout="${GC_BEADS_PULL_TIMEOUT_SECS-1800}"
...
if [ "$pull_timeout_valid" != true ]; then
  printf 'gc dolt pull: invalid GC_BEADS_PULL_TIMEOUT_SECS=%s (must be a positive integer)\n' \
    "$pull_timeout" >&2
  exit 2
fi
