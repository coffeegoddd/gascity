...
# GC_BEADS_HOST) the message names the remote endpoint rather than a managed
...
#
# Environment: GC_CITY_PATH, GC_BEADS_HOST, GC_BEADS_PORT
set -e
...
if GC_CITY_PATH="$GC_CITY_PATH" "$GC_BEADS_BD_SCRIPT" probe >/dev/null 2>&1; then
  if is_local_dolt_host "$host"; then
    echo "Dolt server: running (managed, 127.0.0.1:$GC_BEADS_PORT)"
  else
    echo "Dolt server: reachable (external endpoint $host:$GC_BEADS_PORT)"
  fi
  exit 0
fi

if is_local_dolt_host "$host"; then
  echo "Dolt server: not running (managed, 127.0.0.1:$GC_BEADS_PORT)"
else
  echo "Dolt server: unreachable (external endpoint $host:$GC_BEADS_PORT)"
fi
exit 1
